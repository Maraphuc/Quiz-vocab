...
class Topic {
  const Topic({required this.id, required this.title, required this.titleVi, required this.icon, required this.level, required this.description});
  final String id;
  final String title;
...
class VocabWord {
  const VocabWord({required this.id, required this.word, required this.meaning, required this.pronunciation, required this.example, required this.topicId, required this.level, required this.wrongOptions});
  final String id;
  final String word;
...
class QuizQuestion {
  const QuizQuestion({required this.id, required this.word, required this.type, required this.prompt, required this.options, required this.correctAnswer, this.subPrompt, this.explanation});
  final String id;
...
class WrongWordRecord {
  WrongWordRecord({required this.wordId, required this.wrongCount, required this.lastWrongAt});
  final String wordId;
  int wrongCount;
  String lastWrongAt;
  Map<String, dynamic> toJson() => {'wordId': wordId, 'wrongCount': wrongCount, 'lastWrongAt': lastWrongAt};
  static WrongWordRecord fromJson(Map<String, dynamic> json) => WrongWordRecord(wordId: json['wordId'] as String? ?? '', wrongCount: (json['wrongCount'] as num?)?.toInt() ?? 1, lastWrongAt: json['lastWrongAt'] as String? ?? DateTime.now().toIso8601String());
}
...
class TopicStat {
  ...
  int completedCount;
  int highestScore;
  Map<String, dynamic> toJson() => {'completedCount': completedCount, 'highestScore': highestScore};
  static TopicStat fromJson(Map<String, dynamic> json) => TopicStat(completedCount: (json['completedCount'] as num?)?.toInt() ?? 0, highestScore: (json['highestScore'] as num?)?.toInt() ?? 0);
}

class UserProgress {
  UserProgress({this.totalQuizzes = 0, this.totalCorrect = 0, this.totalQuestions = 0, this.highScore = 0, this.streakDays = 1, String? lastActiveDate, Map<String, TopicStat>? topicStats, List<WrongWordRecord>? wrongWords})
  ...
  Map<String, dynamic> toJson() => {'totalQuizzes': totalQuizzes, 'totalCorrect': totalCorrect, 'totalQuestions': totalQuestions, 'highScore': highScore, 'streakDays': streakDays, 'lastActiveDate': lastActiveDate, 'topicStats': topicStats.map((key, value) => MapEntry(key, value.toJson())), 'wrongWords': wrongWords.map((record) => record.toJson()).toList()};
  static UserProgress fromJson(Map<String, dynamic> json) {
    ...
    if (rawStats is Map) {
      rawStats.forEach((key, value) {
        if (value is Map) stats[key.toString()] = TopicStat.fromJson(Map<String, dynamic>.from(value));
      });
    }
    ...
    if (rawWrong is List) {
      for (final item in rawWrong) {
        if (item is Map) {
          ...
          if (parsed.wordId.isNotEmpty) wrong.add(parsed);
        }
      }
    }
    return UserProgress(totalQuizzes: (json['totalQuizzes'] as num?)?.toInt() ?? 0, totalCorrect: (json['totalCorrect'] as num?)?.toInt() ?? 0, totalQuestions: (json['totalQuestions'] as num?)?.toInt() ?? 0, highScore: (json['highScore'] as num?)?.toInt() ?? 0, streakDays: (json['streakDays'] as num?)?.toInt() ?? 1, lastActiveDate: json['lastActiveDate'] as String? ?? todayString(), topicStats: stats, wrongWords: wrong);
  }
}

class QuizResultData {
  const QuizResultData({required this.topicId, required this.topicTitle, required this.correctCount, required this.totalQuestions, required this.wrongWords, required this.percent});
  final String topicId;
...
final vocabRows = <String, List<List<String>>>{
  ...
  'business': [
    ['Revenue', 'Doanh thu', '/ˈrev.ə.nuː/', 'The company ___ increased.', 'B2', 'Hành lý|Triệu chứng|Món ăn'],
  ...
  'environment': [
    ...
    ['Habitat', 'Môi trường sống', '/ˈhæb.ə.tæt/', 'Bamboo forests are a panda ___.', 'B1', 'Hóa đơn|Bài tập|Khuôn viên'],
  ],
  'sports': [
...
final vocabData = vocabRows.entries.expand((entry) => List.generate(entry.value.length, (index) {
      final row = entry.value[index];
      return VocabWord(id: '${entry.key}_${index + 1}', word: row[0], meaning: row[1], pronunciation: row[2], example: row[3], topicId: entry.key, level: row[4], wrongOptions: row[5].split('|'));
    })).toList();
...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  static const progressKey = 'quiz_vocab_progress_v4';
  ...
  Future<void> _loadState() async {
    ...
    try {
      loadedProgress = raw == null ? UserProgress() : UserProgress.fromJson(jsonDecode(raw) as Map<String, dynamic>);
    } catch (_) {
      loadedProgress = UserProgress();
    }
  ...
  Future<void> _saveProgress() async => prefs?.setString(progressKey, jsonEncode(progress.toJson()));
  ...
  List<QuizQuestion> _generateQuestions(List<VocabWord> words) {
    ...
    return pairs.take(min(20, pairs.length)).toList().asMap().entries.map((entry) => _questionFromPair(entry.value.key, entry.value.value, entry.key, random)).toList();
  }
  ...
  QuizQuestion _questionFromPair(VocabWord word, QuestionType type, int index, Random random) {
    ...
    if (type == QuestionType.meaning || type == QuestionType.context) {
      ...
      return QuizQuestion(id: '${word.id}_${type.name}_$index', word: word, type: type, prompt: type == QuestionType.meaning ? 'Nghĩa tiếng Việt của "${word.word}" là gì?' : 'Trong câu dưới đây, "${word.word}" mang nghĩa gì?', subPrompt: type == QuestionType.meaning ? word.pronunciation : fullSentence, options: options, correctAnswer: word.meaning, explanation: '"${word.word}" có nghĩa là "${word.meaning}". Ví dụ: $fullSentence');
    }
    if (type == QuestionType.reverse || type == QuestionType.pronunciation) {
      final options = _makeOptions(word.word, pool.where((item) => item.id != word.id).map((item) => item.word), random);
      return QuizQuestion(id: '${word.id}_${type.name}_$index', word: word, type: type, prompt: type == QuestionType.reverse ? 'Từ tiếng Anh nào mang nghĩa "${word.meaning}"?' : 'Từ nào có phiên âm ${word.pronunciation}?', subPrompt: type == QuestionType.pronunciation ? 'Gợi ý nghĩa: ${word.meaning}' : null, options: options, correctAnswer: word.word, explanation: '"${word.meaning}" trong tiếng Anh là "${word.word}" ${word.pronunciation}.');
    }
    final options = _makeOptions(word.word, pool.where((item) => item.id != word.id).map((item) => item.word), random);
    return QuizQuestion(id: '${word.id}_${type.name}_$index', word: word, type: type, prompt: 'Chọn từ phù hợp để điền vào chỗ trống:', subPrompt: blankSentence, options: options, correctAnswer: word.word, explanation: 'Câu hoàn chỉnh: $fullSentence');
  }
  ...
  List<String> _makeOptions(String correct, Iterable<String> wrongPool, Random random) {
    ...
    final fallback = [...vocabData.map((word) => word.meaning), ...vocabData.map((word) => word.word)]..shuffle(random);
    for (final option in fallback) {
      if (options.length == 4) break;
      if (option != correct && !options.contains(option)) options.add(option);
    }
  ...
  Future<void> _finishQuiz(String topicId, int correctCount, List<VocabWord> wrongWords) async {
    ...
    final result = QuizResultData(topicId: topicId, topicTitle: activeTopicTitle, correctCount: correctCount, totalQuestions: total, wrongWords: wrongWords, percent: percent);
  ...
  Widget build(BuildContext context) {
    final screens = <Widget>[
      HomeScreen(progress: progress, onStartLearning: () => setState(() => selectedIndex = 1), onViewProgress: () => setState(() => selectedIndex = 3)),
      activeQuestions == null
          ? TopicsScreen(progress: progress, onSelectTopic: _startTopicQuiz)
          : lastResult == null
              ? QuizScreen(topicTitle: activeTopicTitle, questions: activeQuestions!, soundEnabled: soundEnabled, autoSpeak: autoSpeak, onSpeak: _speak, onExit: () => setState(() => activeQuestions = null), onFinish: (correct, wrong) => _finishQuiz(activeTopicId, correct, wrong))
              : ResultScreen(result: lastResult!, onRetry: () {
      ...
      WrongWordsScreen(records: progress.wrongWords, onRemove: _removeWrongWord, onStartQuiz: _startWrongWordsQuiz, onSpeak: _speak),
      ProgressScreen(progress: progress),
      SettingsScreen(soundEnabled: soundEnabled, autoSpeak: autoSpeak, darkMode: darkMode, onSoundChanged: (value) async {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Quiz Vocab', style: TextStyle(fontWeight: FontWeight.w900)), actions: [Padding(padding: const EdgeInsets.only(right: 12), child: Chip(avatar: const Icon(Icons.local_fire_department_rounded, size: 18), label: Text('${progress.streakDays} ngày')))]),
      body: AnimatedSwitcher(duration: const Duration(milliseconds: 250), child: screens[selectedIndex]),
      bottomNavigationBar: NavigationBar(selectedIndex: selectedIndex, onDestinationSelected: (index) {
        setState(() {
          selectedIndex = index;
          if (index != 1) {
            activeQuestions = null;
...
class HomeScreen extends StatelessWidget {
  const HomeScreen({super.key, required this.progress, required this.onStartLearning, required this.onViewProgress});
  final UserProgress progress;
  final VoidCallback onStartLearning;
  final VoidCallback onViewProgress;
  @override
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(20), children: [
        Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF4F46E5), Color(0xFF22C55E)]), borderRadius: BorderRadius.circular(32)), child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(Icons.menu_book_rounded, size: 56, color: Colors.white), SizedBox(height: 16), Text('Học từ vựng thông minh', style: TextStyle(fontSize: 28, color: Colors.white, fontWeight: FontWeight.w900)), SizedBox(height: 8), Text('16 chủ đề, 160 từ vựng và 5 kiểu câu hỏi để luyện hằng ngày.', style: TextStyle(color: Colors.white70, fontSize: 15))])),
        const SizedBox(height: 20),
        Row(children: [Expanded(child: StatCard(title: 'Quiz', value: '${progress.totalQuizzes}', icon: Icons.quiz_rounded)), const SizedBox(width: 12), Expanded(child: StatCard(title: 'Độ chính xác', value: '${(progress.accuracy * 100).round()}%', icon: Icons.check_circle_rounded))]),
        const SizedBox(height: 12),
        Row(children: [Expanded(child: StatCard(title: 'Chủ đề', value: '${topics.length}', icon: Icons.category_rounded)), const SizedBox(width: 12), Expanded(child: StatCard(title: 'Từ vựng', value: '${vocabData.length}', icon: Icons.translate_rounded))]),
        const SizedBox(height: 20),
...
class StatCard extends StatelessWidget {
  ...
  final IconData icon;
  @override
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, color: Theme.of(context).colorScheme.primary), const SizedBox(height: 12), Text(value, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900)), Text(title, style: Theme.of(context).textTheme.bodySmall)])));
}
...
class TopicsScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => ListView.separated(padding: const EdgeInsets.all(18), itemCount: topics.length, separatorBuilder: (_, __) => const SizedBox(height: 12), itemBuilder: (context, index) {
        final topic = topics[index];
        final count = vocabData.where((word) => word.topicId == topic.id).length;
        final stat = progress.topicStats[topic.id];
        return Card(child: ListTile(contentPadding: const EdgeInsets.all(16), leading: CircleAvatar(child: Icon(topic.icon)), title: Text('${topic.title} • ${topic.titleVi}', style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Padding(padding: const EdgeInsets.only(top: 6), child: Text('${topic.description}\n$count từ • tối đa 20 câu/lượt • ${topic.level}${stat == null ? '' : '\nĐã học ${stat.completedCount} lần • cao nhất ${stat.highestScore}%'}')), isThreeLine: true, trailing: const Icon(Icons.chevron_right_rounded), onTap: () => onSelectTopic(topic)));
      });
}

class QuizScreen extends StatefulWidget {
  const QuizScreen({super.key, required this.topicTitle, required this.questions, required this.soundEnabled, required this.autoSpeak, required this.onSpeak, required this.onExit, required this.onFinish});
  final String topicTitle;
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  void _autoSpeak() {
    if (widget.soundEnabled && widget.autoSpeak) widget.onSpeak(current.word.word);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(18), children: [
      Row(children: [IconButton(onPressed: widget.onExit, icon: const Icon(Icons.close_rounded)), Expanded(child: Text(widget.topicTitle, style: const TextStyle(fontWeight: FontWeight.w800), textAlign: TextAlign.center)), Chip(label: Text('$score/${widget.questions.length}'))]),
      const SizedBox(height: 8),
      LinearProgressIndicator(value: progress),
      const SizedBox(height: 16),
      Card(child: Padding(padding: const EdgeInsets.all(22), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Câu ${currentIndex + 1}/${widget.questions.length}', style: Theme.of(context).textTheme.labelLarge), const SizedBox(height: 12), Text(current.prompt, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900)), if (current.subPrompt != null) ...[const SizedBox(height: 12), Container(width: double.infinity, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Theme.of(context).colorScheme.primaryContainer, borderRadius: BorderRadius.circular(16)), child: Text(current.subPrompt!, style: TextStyle(color: Theme.of(context).colorScheme.onPrimaryContainer)))], const SizedBox(height: 12), OutlinedButton.icon(onPressed: () => widget.onSpeak(current.word.word), icon: const Icon(Icons.volume_up_rounded), label: const Text('Nghe phát âm'))]))),
      const SizedBox(height: 12),
      for (final option in current.options) ...[AnswerButton(option: option, answered: answered, selected: selected == option, correct: option == current.correctAnswer, onPressed: () => _answer(option)), const SizedBox(height: 10)],
      if (answered) ...[const SizedBox(height: 8), Card(child: Padding(padding: const EdgeInsets.all(16), child: Text(current.explanation ?? '', style: const TextStyle(fontWeight: FontWeight.w600)))), const SizedBox(height: 12), FilledButton.icon(onPressed: _next, icon: const Icon(Icons.arrow_forward_rounded), label: Text(currentIndex + 1 == widget.questions.length ? 'Xem kết quả' : 'Câu tiếp theo'))],
    ]);
  }
}

class AnswerButton extends StatelessWidget {
  const AnswerButton({super.key, required this.option, required this.answered, required this.selected, required this.correct, required this.onPressed});
  final String option;
  final bool answered;
  final bool selected;
  final bool correct;
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(duration: const Duration(milliseconds: 180), child: Material(color: background ?? Theme.of(context).cardColor, borderRadius: BorderRadius.circular(18), child: InkWell(borderRadius: BorderRadius.circular(18), onTap: answered ? null : onPressed, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16), child: Row(children: [Expanded(child: Text(option, style: const TextStyle(fontWeight: FontWeight.w800))), if (icon != null) Icon(icon, color: correct ? Colors.green : Colors.red)])))));
  }
}

class ResultScreen extends StatelessWidget {
  const ResultScreen({super.key, required this.result, required this.onRetry, required this.onOtherTopic, required this.onWrongWords});
  final QuizResultData result;
  final VoidCallback onRetry;
  final VoidCallback onOtherTopic;
  final VoidCallback onWrongWords;
  String get rating => result.percent >= 85 ? 'Xuất sắc' : result.percent >= 60 ? 'Tốt' : 'Cần luyện thêm';
  @override
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(22), children: [Card(child: Padding(padding: const EdgeInsets.all(24), child: Column(children: [const Icon(Icons.emoji_events_rounded, size: 72, color: Colors.amber), const SizedBox(height: 16), Text(rating, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900)), const SizedBox(height: 8), Text(result.topicTitle, textAlign: TextAlign.center), const SizedBox(height: 16), Text('${result.correctCount}/${result.totalQuestions} đúng • ${result.percent}%', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800)), Text('Từ sai: ${result.wrongWords.length}')]))), const SizedBox(height: 16), FilledButton.icon(onPressed: onRetry, icon: const Icon(Icons.refresh_rounded), label: const Text('Làm lại')), const SizedBox(height: 10), OutlinedButton.icon(onPressed: onOtherTopic, icon: const Icon(Icons.category_rounded), label: const Text('Chọn chủ đề khác')), const SizedBox(height: 10), OutlinedButton.icon(onPressed: onWrongWords, icon: const Icon(Icons.replay_rounded), label: const Text('Xem từ sai'))]);
}

class WrongWordsScreen extends StatelessWidget {
  const WrongWordsScreen({super.key, required this.records, required this.onRemove, required this.onStartQuiz, required this.onSpeak});
  ...
  Widget build(BuildContext context) {
    ...
    }).where((entry) => entry.value != null).toList()..sort((a, b) => b.key.wrongCount.compareTo(a.key.wrongCount));
    if (words.isEmpty) {
      return Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.check_circle_rounded, size: 72, color: Theme.of(context).colorScheme.primary), const SizedBox(height: 16), const Text('Chưa có từ sai', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900)), const SizedBox(height: 8), const Text('Hãy làm quiz để hệ thống tự lưu các từ cần ôn lại.', textAlign: TextAlign.center)])));
    }
    return ListView(padding: const EdgeInsets.all(18), children: [Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Ôn tập từ sai', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900)), const SizedBox(height: 8), Text('${words.length} từ cần ôn lại. Ưu tiên các từ sai nhiều lần trước.'), const SizedBox(height: 14), FilledButton.icon(onPressed: onStartQuiz, icon: const Icon(Icons.play_arrow_rounded), label: const Text('Làm quiz từ sai'))]))), const SizedBox(height: 12), for (final entry in words) _WrongWordCard(record: entry.key, word: entry.value!, onRemove: onRemove, onSpeak: onSpeak)]);
  }
}

class _WrongWordCard extends StatelessWidget {
  const _WrongWordCard({required this.record, required this.word, required this.onRemove, required this.onSpeak});
  final WrongWordRecord record;
  final VocabWord word;
  final ValueChanged<String> onRemove;
  final ValueChanged<String> onSpeak;
  @override
  Widget build(BuildContext context) {
    final fullExample = word.example.replaceAll('___', word.word);
    return Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Expanded(child: Text(word.word, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900))), Chip(label: Text('Sai ${record.wrongCount} lần'))]), const SizedBox(height: 6), Text(word.pronunciation, style: TextStyle(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.w700)), const SizedBox(height: 8), Text(word.meaning, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700)), const SizedBox(height: 8), Text(fullExample), const SizedBox(height: 8), Wrap(spacing: 8, runSpacing: 8, children: [Chip(label: Text(word.level)), OutlinedButton.icon(onPressed: () => onSpeak(word.word), icon: const Icon(Icons.volume_up_rounded), label: const Text('Nghe')), TextButton.icon(onPressed: () => onRemove(record.wordId), icon: const Icon(Icons.delete_outline_rounded), label: const Text('Đã thuộc'))])])));
  }
}
...
class ProgressScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(18), children: [Row(children: [Expanded(child: StatCard(title: 'Quiz đã làm', value: '${progress.totalQuizzes}', icon: Icons.quiz_rounded)), const SizedBox(width: 12), Expanded(child: StatCard(title: 'Điểm cao', value: '${progress.highScore}%', icon: Icons.star_rounded))]), const SizedBox(height: 12), Row(children: [Expanded(child: StatCard(title: 'Câu đúng', value: '${progress.totalCorrect}', icon: Icons.check_circle_rounded)), const SizedBox(width: 12), Expanded(child: StatCard(title: 'Từ sai', value: '${progress.wrongWords.length}', icon: Icons.replay_rounded))]), const SizedBox(height: 20), const Text('Tiến độ theo chủ đề', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), const SizedBox(height: 8), for (final topic in sortedTopics) Card(child: ListTile(leading: Icon(topic.icon), title: Text(topic.titleVi), subtitle: Text('Đã học: ${progress.topicStats[topic.id]?.completedCount ?? 0} lần'), trailing: Text('${progress.topicStats[topic.id]?.highestScore ?? 0}%')))]);
  }
}

class SettingsScreen extends StatelessWidget {
  const SettingsScreen({super.key, required this.soundEnabled, required this.autoSpeak, required this.darkMode, required this.onSoundChanged, required this.onAutoSpeakChanged, required this.onDarkModeChanged, required this.onResetProgress});
  final bool soundEnabled;
  final bool autoSpeak;
  final bool darkMode;
  ...
  Future<void> _confirmReset(BuildContext context) async {
    final confirmed = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(title: const Text('Xóa toàn bộ tiến độ?'), content: const Text('Hành động này sẽ xóa điểm, streak, tiến độ theo chủ đề và danh sách từ sai. Bạn không thể hoàn tác sau khi xóa.'), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Hủy')), FilledButton.tonalIcon(onPressed: () => Navigator.pop(dialogContext, true), icon: const Icon(Icons.delete_outline_rounded), label: const Text('Xóa tiến độ'))]));
    if (confirmed == true && context.mounted) {
      onResetProgress();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Đã reset tiến độ học tập.')));
    }
  }

  @override
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(18), children: [SwitchListTile(value: soundEnabled, onChanged: onSoundChanged, title: const Text('Bật âm thanh/phát âm'), secondary: const Icon(Icons.volume_up_rounded)), SwitchListTile(value: autoSpeak, onChanged: onAutoSpeakChanged, title: const Text('Tự đọc từ khi sang câu mới'), secondary: const Icon(Icons.record_voice_over_rounded)), SwitchListTile(value: darkMode, onChanged: onDarkModeChanged, title: const Text('Dark mode'), secondary: const Icon(Icons.dark_mode_rounded)), const Divider(), ListTile(leading: const Icon(Icons.privacy_tip_rounded), title: const Text('Chính sách quyền riêng tư'), subtitle: const Text('App không thu thập dữ liệu cá nhân. Tiến độ lưu trên thiết bị.'), onTap: () => showDialog<void>(context: context, builder: (context) => AlertDialog(title: const Text('Privacy Policy'), content: const SingleChildScrollView(child: Text('Quiz Vocab hoạt động offline, không yêu cầu đăng nhập, không thu thập tên, email, vị trí, danh bạ, ảnh hoặc dữ liệu cá nhân. Điểm số, streak và danh sách từ sai chỉ được lưu cục bộ trên thiết bị bằng SharedPreferences. Người dùng có thể xóa dữ liệu bằng nút Reset tiến độ hoặc gỡ ứng dụng.')), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Đã hiểu'))]))), ListTile(leading: const Icon(Icons.restart_alt_rounded), title: const Text('Reset tiến độ'), subtitle: const Text('Xóa điểm, streak và danh sách từ sai'), onTap: () => _confirmReset(context)), const SizedBox(height: 20), const Center(child: Text('Quiz Vocab v1.1.0 • 16 chủ đề • 160 từ vựng'))]);
}
